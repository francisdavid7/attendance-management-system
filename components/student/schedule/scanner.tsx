...
export default function QRScanner() {
  ...
  const qRCode = useRef(null) as any;
    ...
    try {
      ...
      await scannerRef.current.start(
        ...
        (qrCode) => {
          ...
          qRCode.current = data;
          // console.log("QR-CODE", qRCode.current.id, qRCode.current.qrCode);
          stopScanning();
        },
        () => {},
      );
  ...
  const submitAttendance = async () => {
    if (!qRCode) return alert("No code to mark");
    const id = qRCode?.current?.id;
    const qrCode = qRCode?.current?.qrCode;
              ...
              <div className="text-center mt-3 justify-center justify-self-center">
                {!isScanning ? (
                  <>
                    {!qRCode &&
                      (disPlay ? (
                        <span className="text-(--color-destructive) font-semibold">
                          <span>
                            {" "}
                            Scan the QR code to mark session attendance{" "}
                          </span>
                          <CircleAlert
                            size={100}
                            className="text-center mt-3 mb-2 justify-center justify-self-center"
                          />
                          No QR code scanned
                        </span>
                      ) : (
                    ...
                    {qRCode && (
                      <span className="text-(--color-primary) font-semibold">
                        ...
                        <span className="">
                          {" "}
                          QR Code Scanned Ready to Submit
                        </span>
                      </span>
                    )}
                  </>
                ...
                <div id="reader" className="mt-3">
                  <span>
                    {!disPlay ? (
                      <ScanLine
                        size={100}
                        className="text-center mt-3 justify-center justify-self-center"
                      />
                    ) : (
                      ""
                    )}
                ...
                {!isScanning ? (
                  <div className="grid md:flex gap-1 ">
                    <div>
                      <Button
                        variant="default"
                        className=" rounded-xl p-5 asChild "
                        onClick={toggleScanner}
                      >
                        {isScanning ? (
                          <>
                            <StopCircle /> Stop Scanning QRcode{" "}
                          </>
                        ) : (
                          <>
                            {" "}
                            <LucideSquareCenterlineDashedVertical /> Start
                            Scanning QRcode{" "}
                          </>
                        )}
                      </Button>
                    ...
                    <div>
                      <DialogClose asChild>
                        <Button
                          variant="default"
                          className=" rounded-xl  px-5 asChild  "
                          disabled={!qRCode}
